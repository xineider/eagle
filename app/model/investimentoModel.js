	...
	GetInvestimentos(id_usuario) {
		return new Promise(function(resolve, reject) {
			helper.Query('SELECT REPLACE(ROUND((\
				...
				GROUP BY a.id_plano \
				ORDER BY b.id', [0,0,id_usuario,1,2,0,id_usuario,1,1,0,id_usuario,1,3,0,id_usuario,1,2,0,id_usuario,1,3,0,id_usuario,1,0,id_usuario,1]).then(data => {
					resolve(data);
				});
			});
	}
