...
var PlanoModel = require('../model/planoModel.js');
var model = new PlanoModel;
var data = {};
var app = express();
	...
	model.GetUsuario(req.session.usuario.id).then(data_perfil=>{
		...
		model.GetAporte(req.session.usuario.id).then(data_aporte_todal=>{
			...
			model.GetPrimeiroAporte(req.session.usuario.id).then(data_primeiro_aporte=>{
				...
				console.log(data);
				console.log('=======================================================');
				res.render(req.isAjaxRequest() == true ? 'api' : 'montador', {html: 'plano/plano', data: data, usuario: req.session.usuario});
			});
		});
	});
});
