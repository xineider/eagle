...
router.get('/', function(req, res, next) {
	model.SelecionarUsuarios().then(data => {
		res.render(req.isAjaxRequest() == true ? 'api' : 'montadorMobile', {html: 'usuarios/index', data: data, usuario: req.session.usuario});
	});
});
router.get('/criar', function(req, res, next) {
	model.SelecionarSetores().then(data_setores => {
		...
		console.log(data);
		res.render(req.isAjaxRequest() == true ? 'api' : 'montadorMobile', {html: 'usuarios/usuarios_criar', data: data, usuario: req.session.usuario});
	});
});
...
router.get('/editar/perfil/:id', function(req, res, next) {
	model.SelecionarUsuario(req.params.id).then(data_usuario => {
		data['usuario'] = data_usuario;
		res.render(req.isAjaxRequest() == true ? 'api' : 'montadorMobile', {html: 'usuarios/usuarios_perfil_editar', data: data, usuario: req.session.usuario});
	});
});
...
// POSTS
	...
	router.post('/ver/perfil/', function(req, res, next) {
		...
		model.LoadPerfil(POST.id).then(data => {
			res.render(req.isAjaxRequest() == true ? 'api' : 'montadorMobile', {html: 'usuarios/usuarios_perfil_header', data: data});
		});
	});
